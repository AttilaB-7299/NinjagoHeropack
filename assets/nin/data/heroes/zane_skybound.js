...
function spinjitzuAttack(hero, entity, manager) {
    if (spin(entity) && entity.getData("fiskheroes:beam_charging")) {
        ...
        for (var i = 0; i < list.size(); ++i) {
            ...
            if (other.isLivingEntity() && !entity.equals(other) && entity.world().isUnobstructed(entity.pos().add(0, 1, 0), other.pos().add(0, 1, 0))) {
                other.hurtByAttacker(hero, "COLD", "%s was ripped to shreads by %s's spinjitzu", 35, entity);
            }
        }
    }
    else if (spin(entity) && entity.getData("fiskheroes:beam_charging") && entity.getHeldItem().nbt().getString("WeaponType") == "nin:shuriken") {
        var list = entity.world().getEntitiesInRangeOf(entity.pos(), 2.5);
        for (var i = 0; i < list.size(); ++i) {
            var other = list.get(i);
            if (other.isLivingEntity() && !entity.equals(other) && entity.world().isUnobstructed(entity.pos().add(0, 1, 0), other.pos().add(0, 1, 0))) {
                other.hurtByAttacker(hero, "COLD", "%s was turned into a popsicle by %s's spinjitzu", 50, entity);
            }
        }
    }
}
