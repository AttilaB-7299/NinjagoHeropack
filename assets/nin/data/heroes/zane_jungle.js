...
function init(hero) {
    ...
    hero.addKeyBind("SHAPE_SHIFT", "Disguise", 2);
    hero.addKeyBind("SHAPE_SHIFT_RESET", "Reset Disguise", 2);
...
function spinjitzuAttack(hero, entity, manager) {
    if (spin(entity) && entity.getData("nin:dyn/spinning_1") && entity.getHeldItem().nbt().getString("WeaponType") != "nin:shuriken") {
    ...
    else if (spin(entity) && entity.getData("nin:dyn/spinning_1") && entity.getHeldItem().nbt().getString("WeaponType") == "nin:shuriken") {
        var list = entity.world().getEntitiesInRangeOf(entity.pos(), 2.5);
        for (var i = 0; i < list.size(); ++i) {
            var other = list.get(i);
            if (other.isLivingEntity() && !entity.equals(other) && entity.world().isUnobstructed(entity.pos().add(0, 1, 0), other.pos().add(0, 1, 0))) {
...
function isKeyBindEnabled(entity, keyBind) {
    ...
    switch (keyBind) {
        //powerset 1
        case "SHAPE_SHIFT":
            return (entity.getData("nin:dyn/powerset") == 2) && !entity.isSneaking();
